...
public class ScreenLog : DontDestroySingletonMonoBehaviour<ScreenLog>
{
    private Rect rect;
	float time;
	GUIStyle style;
	...
	{
	#if GAME_DEBUG
		logMsg.Enqueue(msg.ToString());
	#endif
	}
	public static void LogFormat(string format, object msg)
	{
	#if GAME_DEBUG
		logMsg.Enqueue(string.Format(format, msg));
	#endif
	}
